...
class AAAASetupDatabase(TestCase):
    ...
    def test_setup_setup_database_load_stored_proc(self):
        ...
        self.assertContains(self.response, 'push_sp_anzahl_import_elemente war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_vorbereitung war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_neue_user war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_behandle_user war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_behandle_rechte war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_loesche_doppelte_rechte war erfolgreich.', 1)
        self.assertNotContains(self.response, 'push_sp_nichtai war erfolgreich.')
        self.assertContains(self.response, 'push_sp_erzeuge_af_liste war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_ueberschreibe_modelle war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_direct_connects war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_af_umbenennen war erfolgreich.', 1)
        self.assertContains(self.response, 'push_sp_ungenutzte_teams war erfolgreich.', 1)
...
class PanelExportCSVTest(TestCase):
    ...
    def test_panel_online_without_selection(self):
        ...
        self.assertContains(response,
                            "Name\tTeam\tTF\tTF Beschreibung\tAF\tAF Beschreibung\tHöchste Kritikalitaet TFin AF\tGF\tgf_beschreibung\tZI-Orga\tPlattform\tTF Kritikalität\tRecht aktiv\tUser aktiv\tloeschdatum\taf_gueltig_ab\taf_gueltig_bis\tdirect_connect\taf_zuweisungsdatum\ttf_eigentuemer_org\tgefunden\twiedergefunden\tletzte_aenderung\tAF Neu\tGF Neu\r",
                            1)
...
class UserRolleNurNeueAFTest(TestCase):
    ...
    def test_panel_03_view_with_valid_role_star_name_and_group(self):
        """
        Zum Vergleich wird hier nochmal geprüft, ob die "*"-Anzeige für die geänderten Userelemente korrekt ist
        """
        url = '{0}{1}'.format(reverse('user_rolle_af'), '?rollenname=-&name=UseR&gruppe=BA-ls')
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_panel_05_view_with_valid_role_dash_unique_name_and_group_not_containing_new_AF(self):
        """
        Suche nach den nicht zugewiesenen AFen bei einem vorgegebenen User, der so etwas nicht enthält.
        """
        ...
        self.assertContains(response, "Keine AF zugewiesen oder durch Filter ausgeblendet", 2)
